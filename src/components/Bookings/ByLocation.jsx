...
function LocationBookingList() {
    ...
    const fetchBookings = useCallback(async () => {
        try {
            const { data } = await axios.get(`${bookings}/location/${params.id}?page=${pagination.page}`);
            ...
            swal('Failed', error.response.data.message, 'error').then(() => history.push('/locations'));
        }
    }, [pagination.page]);
    ...
    return (
        <div className='users container w-75 m-auto mt-5'>
            <div className='row mb-3'>
                <div className='col-6'>
                    <h2>Bookings for {location.title}</h2>
                </div>
                {/* <div className='col-9'>
...
export default LocationBookingList;
...
function UserBookingList() {
    ...
    const fetchBookings = useCallback(async () => {
        try {
            const { data } = await axios.get(`${bookings}/user/${params.id}?page=${pagination.page}`);
            ...
            swal('Failed', error.response.data.message, 'error').then(() => history.push('/users'));
        }
    }, [pagination.page]);
    ...
    return (
        <div className='users container w-75 m-auto mt-5'>
            <div className='row mb-3'>
                <div className='col-6'>
                    <h2>Bookings for {user.firstName} {user.lastName}</h2>
                </div>
                {/* <div className='col-9'>
